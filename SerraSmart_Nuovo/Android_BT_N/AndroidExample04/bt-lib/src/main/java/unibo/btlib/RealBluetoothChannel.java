...
public final class RealBluetoothChannel extends BluetoothChannel {
    ...
    @Override
    public void close() {
        running = false;  // Ferma il worker
        worker.cancel();  // Chiude il socket
        super.close();
    }
    ...
    class BluetoothWorker implements ExtendedRunnable {
        ...
        public void run() {
            ...
            while (running) {
                try {
                    ...
                    while (running && (inputByte = input.readByte()) != 0) {
                    ...
                    if (running) {
                        Log.e(C.LIB_TAG, "Error reading from input stream", e);
                    } else {
                        Log.d(C.LIB_TAG, "BluetoothWorker stopped");
                    }
                    break;
                }
            }
        ...
        public void write(byte[] bytes) {
            try {
                ...
                Log.e(C.LIB_TAG, "Error writing to output stream", e);
            }
        }
